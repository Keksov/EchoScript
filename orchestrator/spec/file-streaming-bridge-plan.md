...
- **SB-D3 — Окно = 30 c pcm16le (960000 байт)**, один бинарный кадр на окно, `flush` после каждого
  окна. Совпадает с внутренним окном whisper; редундантная переинференция ограничена границами
  предложений. Размер — конфигурируемый, обсуждается на ревью P0.
- **SB-D4 — Единый путь.** Все файловые задачи идут через мост; `transcribeFileViaDaemon` (one-shot)
  остаётся только для юнит-тестов/обратной совместимости, из джоб-раннера убирается.
...
- [ ] **SB0.1 — Plan & ledger.** Создать оба файла, DAG ацикличен, атомарный коммит.
...
- [ ] **SB2.1 — progress.json в раннере + переключение на мост.** Раннер пишет `progress.json` через
  `onProgress`, `runWsDaemonJob` вызывает `transcribeFileStreaming`.
