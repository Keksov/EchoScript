@echo off
REM ============================================================
REM EchoScript — Setup all services and orchestrator
REM ============================================================
setlocal

set "SCRIPT_DIR=%~dp0"
call "%SCRIPT_DIR%env.bat"

echo ============================================================
echo  EchoScript — Full Setup
echo ============================================================
echo.

echo --- [1/9] Orchestrator (Bun) ---
call "%SCRIPT_DIR%..\orchestrator\scripts\setup_orchestrator.bat"
if errorlevel 1 echo [WARN] Orchestrator setup had issues. & echo.

echo --- [2/9] Whisper Podlodka ---
call "%SCRIPT_DIR%..\services\whisper_podlodka\scripts\setup_whisper_podlodka.bat"
if errorlevel 1 echo [WARN] whisper_podlodka setup had issues. & echo.

echo --- [3/9] Borealis ---
call "%SCRIPT_DIR%..\services\borealis\scripts\setup_borealis.bat"
if errorlevel 1 echo [WARN] borealis setup had issues. & echo.

echo --- [4/9] Gemma4 ---
call "%SCRIPT_DIR%..\services\gemma4\scripts\setup_gemma4.bat"
if errorlevel 1 echo [WARN] gemma4 setup had issues. & echo.

echo --- [5/9] VibeVoice ---
call "%SCRIPT_DIR%..\services\vibevoice\scripts\setup_vibevoice.bat"
if errorlevel 1 echo [WARN] vibevoice setup had issues. & echo.

echo --- [6/9] VibeVoice Daemon ---
call "%SCRIPT_DIR%..\services\vibevoicedaemon\scripts\setup_vibevoicedaemon.bat"
if errorlevel 1 echo [WARN] vibevoicedaemon setup had issues. & echo.

echo --- [7/9] Vosk RU ---
call "%SCRIPT_DIR%..\services\vosk_ru\scripts\setup_vosk_ru.bat"
if errorlevel 1 echo [WARN] vosk_ru setup had issues. & echo.

echo --- [8/9] Vosk RU Command ---
call "%SCRIPT_DIR%..\services\vosk_ru_cmd\scripts\setup_vosk_ru_cmd.bat"
if errorlevel 1 echo [WARN] vosk_ru_cmd setup had issues. & echo.

echo --- [9/9] Vosk EN ---
call "%SCRIPT_DIR%..\services\vosk_en\scripts\setup_vosk_en.bat"
if errorlevel 1 echo [WARN] vosk_en setup had issues. & echo.

echo ============================================================
echo  Setup complete.
echo  Model weights are NOT downloaded yet.
echo  Use service-local scripts\download_*.bat entrypoints or root grouped download scripts.
echo ============================================================

endlocal
