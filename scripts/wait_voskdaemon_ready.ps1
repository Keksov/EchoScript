...
while ((Get-Date) -lt $deadline) {
    $processes = Get-CimInstance Win32_Process -Filter "Name = 'VoskDaemon.exe'" | Where-Object {
        $_.CommandLine -match $processPattern
    }

    if (@($processes).Count -eq 0) {
        throw "VoskDaemon process for $ModelName is not running"
    }
    ...
    if (Test-Path $stdoutLog) {
        ...
        if ($content -and $content.Contains($readyMarker)) {
            ...
            Write-Host ("[voskdaemon] fully loaded and ready for requests model={0}" -f $ModelName)
            exit 0
        }
    ...
    if (Test-Path $stderrLog) {
        ...
        if ($stderrContent -and $stderrContent.Contains('[voskdaemon] warmup failed:')) {
