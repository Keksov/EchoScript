...
if defined ECHOSCRIPT_PROXY_CONFIGURED exit /b 0
...
:use_windows_proxy
for /f "tokens=1,* delims==" %%A in ('pwsh -NoProfile -Command "$raw = $env:PROXY_RAW; $http = $null; $https = $null; foreach ($part in ($raw -split ';')) { $segment = $part.Trim(); if (-not $segment) { continue } if ($segment -match '^(?<scheme>[^=]+)=(?<value>.+)$') { $scheme = $Matches['scheme'].Trim().ToLowerInvariant(); $value = $Matches['value'].Trim(); if ($value -and $value -notmatch '^[A-Za-z][A-Za-z0-9+.-]*://') { $value = 'http://' + $value }; if ($scheme -eq 'http') { $http = $value } elseif ($scheme -eq 'https') { $https = $value } } elseif (-not $http -and -not $https) { $value = $segment; if ($value -and $value -notmatch '^[A-Za-z][A-Za-z0-9+.-]*://') { $value = 'http://' + $value }; $http = $value; $https = $value } }; if (-not $http) { $http = $https }; if (-not $https) { $https = $http }; if ($http) { Write-Output ('HTTP_PROXY=' + $http) }; if ($https) { Write-Output ('HTTPS_PROXY=' + $https) }"') do set "%%A=%%B"
