...
if not defined RUNTIME_URL (
    ...
    pwsh -NoProfile -Command "$ErrorActionPreference='Stop'; $headers=@{'User-Agent'='EchoScript-WhisperDaemon-Downloader'}; $releaseApi = if ($env:SHERPA_ONNX_RUNTIME_TAG) { 'https://api.github.com/repos/k2-fsa/sherpa-onnx/releases/tags/' + $env:SHERPA_ONNX_RUNTIME_TAG } else { 'https://api.github.com/repos/k2-fsa/sherpa-onnx/releases/latest' }; $release = Invoke-RestMethod -Uri $releaseApi -Headers $headers; $pattern = if ($env:SHERPA_ONNX_RUNTIME_ASSET_PATTERN) { $env:SHERPA_ONNX_RUNTIME_ASSET_PATTERN } else { '^sherpa-onnx-v[\d.]+-win-x64-shared-MT-Release-lib\.tar\.bz2$' }; $asset = $release.assets | Where-Object { $_.name -match $pattern } | Select-Object -First 1; if (-not $asset) { throw ('Could not find runtime asset matching pattern: ' + $pattern) }; Set-Content -LiteralPath $env:RUNTIME_URL_FILE -Value $asset.browser_download_url -NoNewline"
    if errorlevel 1 (
